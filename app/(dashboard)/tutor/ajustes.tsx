...
import { Bell, LogOut, ChevronRight } from 'lucide-react-native';

export default function TutorAjustes() {
    ...
    const handleLogout = () => {
        Alert.alert("Cerrar Sesión", "¿Deseas salir de la cuenta?", [
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.profileSection}>
                <Image
                    source={{ uri: `https://ui-avatars.com/api/?name=${user?.name}&background=${theme.primary.replace('#', '')}&color=fff&size=128` }}
                    style={styles.avatar}
                />
                <Text style={styles.userName}>{user?.name}</Text>
                <Text style={styles.userEmail}>{user?.email}</Text>
                <Text style={styles.userRole}>Tutor / Padre</Text>
            </View>
...
const styles = StyleSheet.create({
    ...
    userRole: { fontSize: 12, fontWeight: '700', color: '#6366F1', marginTop: 4, backgroundColor: '#EEF2FF', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 10 },
